  ...
  const serviciosFiltrados = useMemo(
    () =>
      servicios.filter((s) => {
        ...
        return (
          (!ciudad || s.ciudad.toLowerCase() === ciudad.toLowerCase()) &&
          (!vehiculo ||
            s.categoria_vehiculo.nombre.toLowerCase() ===
              vehiculo.toLowerCase()) &&
          (!empresa || s.empresa.toLowerCase() === empresa.toLowerCase())
        );
      }),
    [servicios, filters]
  );
      ...
      <div className="mb-6 rounded bg-custom-dark p-4">
        <div className="flex flex-col gap-4 md:flex-row md:items-end">
          ...
          <button
            ...
            className="w-fit h-fit  rounded bg-gray-500 px-4 py-2 text-white transition hover:bg-gray-600"
          >
            Limpiar filtros
          </button>
        </div>
