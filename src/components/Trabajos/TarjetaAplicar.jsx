...
const TarjetaAplicar = ({ servicio, onInfo }) => {
  ...
  const handleApply = async () => {
    ...
    const yaEnEsteServicio = existApplication();
    if (yaEnEsteServicio) {
      await Swal.fire(
        "¡Atención!",
        ...
        if (result.isConfirmed) {
          ...
          const found = postulaciones.filter((p) => p.email === user.email);

          const { data } = await axios.get(
            `${API_URL.POSTULACIONES}/${found[0].id}`,
            { headers: { Authorization: `Bearer ${token}` } }
          );
          const documents = data.data.usuario || [];
    ...
    try {
      ...
      await Swal.fire("¡Postulación exitosa!", "Podrás ver todas tus postulaciones en la opción de Postulaciones", "success")
      .then(async () => {
        const found = postulaciones.filter((p) => p.email === user.email);
      ...
      await Swal.fire(
        "Excelente Decisión",
        `Serás redirigido al formulario. Para cargar tus documentos.`,
        "success"
      ).then(async () => {
        const found = postulaciones.filter((p) => p.email === user.email);
        const { data } = await axios.get(
        ...
        navigate("/formulario", { state: { servicio, documents: data.data.usuario } });
      });
    } finally {
  ...
  return (
  <div className="group relative h-64 w-full overflow-hidden rounded shadow transition-transform duration-300 hover:-translate-y-1 hover:shadow-lg">
    <img
      ...
      <div className='flex space-x-2'>
        ...
        <button
          ...
          className={`px-3 py-1 rounded transition-colors ${
            existApplication() && "bg-green-500 hover:bg-green-500/80"
          } ${
            loadingApply
